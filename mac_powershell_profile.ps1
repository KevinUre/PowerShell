...
Import-Module '/Users/kevinure/.local/share/powershell/Modules/posh-git/1.0.0/posh-git.psd1'
...
function prompt {
    ...
    $prompt += Write-Prompt "$($parentPath)" -ForegroundColor yellow #actually white?
    if( ($parentPath.Length -gt 0)`
     -And ($parentPath.Substring($parentPath.Length-1) -ne "/"))
    {
        $prompt += Write-Prompt "/" -ForegroundColor yellow #actually white
    }
    $prompt += Write-Prompt "$(Split-Path $((Get-Location).Path) -Leaf)" -ForegroundColor $global:GitPromptSettings.BeforeStatus.ForegroundColor
